...
export const orderRepo = {
  ...
  findByExternalId: (externalId: string) =>
    prisma.order.findUnique({ where: { externalId } }),
  ...
  update: (id: string, data: any) => prisma.order.update({ where: { id }, data }),

  delete: (id: string) => prisma.order.delete({ where: { id } }),

  createMany: (rows: any[]) =>
    prisma.order.createMany({ data: rows, skipDuplicates: true }),
};
